...
func (s *AchievementService) Get(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(400).JSON(model.ErrorResponse{
			Success: false, Message: "Invalid AchievementId",
		})
	}
	...
	if role == model.RoleMahasiswa {
		...
		if err != nil || ownerID != userID {
			return c.Status(403).JSON(model.ErrorResponse{
				Success: false, Message: "You are not authorised to view other people's achievements.",
			})
		}
	} else if role == model.RoleDosenWali {
		if !s.repo.IsAdvisor(userID, id) {
			return c.Status(403).JSON(model.ErrorResponse{
				Success: false, Message: "You are not the advisor for this student",
			})
		}
	}
...
func (s *AchievementService) Update(c *fiber.Ctx) error {
	...
	if ownerID != userID {
		return c.Status(403).JSON(model.ErrorResponse{
			Success: false, Message: "You are not authorised to update this achievement",
		})
	}
...
func (s *AchievementService) Delete(c *fiber.Ctx) error {
	...
	if ownerID != userID {
		return c.Status(403).JSON(model.ErrorResponse{
			Success: false, Message: "You are not authorised to delete this achievement",
		})
	}
...
func (s *AchievementService) Submit(c *fiber.Ctx) error {
	...
	if ownerID != userID {
		return c.Status(403).JSON(model.ErrorResponse{
			Success: false, Message: "You are not authorised to submit this achievement",
		})
	}
...
func (s *AchievementService) Verify(c *fiber.Ctx) error {
	...
	if currentStatus != "submitted" {
		return c.Status(400).JSON(model.ErrorResponse{
			Success: false, Message: "Achievement must be submitted before verification",
		})
	}
	...
	if req.Points <= 0 {
		return c.Status(400).JSON(model.ErrorResponse{
			Success: false, Message: "Points must be greater than 0",
		})
	}
...
func (s *AchievementService) Reject(c *fiber.Ctx) error {
	...
	if currentStatus != "submitted" {
		return c.Status(400).JSON(model.ErrorResponse{
			Success: false, Message: "Achievement must be submitted before rejection",
		})
	}
	...
	return c.JSON(model.SuccessMessageResponse{
		Success: true, Message: "Rejected Successfully",
	})
}
...
func (s *AchievementService) UploadAttachment(c *fiber.Ctx) error {
	...
	if ownerID != userID {
		return c.Status(403).JSON(model.ErrorResponse{
			Success: false, Message: "You are not authorised to upload attachments for this achievement",
		})
	}
	...
	if err := c.SaveFile(file, path); err != nil {
		return c.Status(500).JSON(model.ErrorResponse{
			Success: false, Message: "Failed saving file",
		})
	}
	...
	if err := s.repo.AddAttachment(id, attachment); err != nil {
		return c.Status(500).JSON(model.ErrorResponse{
			Success: false, Message: "Failed updating db",
		})
	}

	return c.JSON(model.SuccessResponse[*model.Attachment]{
		Success: true, Message: "Attachment uploaded successfully", Data: &attachment,
	})
}
